...
namespace FinancialCrm
{
    public partial class FrmBanks : Form
    {
        ...
        private void FrmBanks_Load(object sender, EventArgs e)
        {
            ...
            //banka bakiyeleri
            var ziraatBankBalance = db.TblBank.Where(x=>x.BankTitle == "Ziraat Bankası").Select(y => y.BankBalance).FirstOrDefault();
            var vakifBankBalance = db.TblBank.Where(x=>x.BankTitle== "VakıfBank").Select(y=>y.BankBalance).FirstOrDefault();
            var isBankBalance = db.TblBank.Where(x=>x.BankTitle== "İş Bankası").Select(y=>y.BankBalance).FirstOrDefault();
